...
export const NavigationSection = (): JSX.Element => {
  ...
  async function handleWalletButtonClick() {
    if (accountId) {
      // Toggle dropdown only when connected
      setMenuOpen((v) => !v);
      return;
    }
    ...
    try {
      ...
      setTimeout(() => (connectingRef.current = false), 300);
    }
  }

  const isBusy = status === "initializing" || status === "connecting";
  const busyTitle =
    status === "initializing" ? "Initializing wallet…" :
    status === "connecting"   ? "Check HashPack…"       :
    undefined;

  return (
    <header className="fixed top-0 left-0 right-0 z-[100] flex items-center justify-between px-5 md:px-8 lg:px-20 h-16 md:h-20 bg-[#0d0d0d30] backdrop-blur-[10px] [-webkit-backdrop-filter:blur(10px)_brightness(100%)] translate-y-[-1rem] animate-fade-in opacity-0">
      {/* Logo */}
      ...
      {/* Primary nav */}
      <nav className="hidden md:flex items-center gap-5 opacity-0 animate-fade-in [--animation-delay:200ms]">
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
      ...
      <div className="flex items-center gap-3 md:gap-5">
        ...
        <div className="relative hidden lg:block w-[420px] xl:w-[526px] opacity-0 animate-fade-in [--animation-delay:400ms]">
          <Input
            type="text"
        ...
        <div className="relative opacity-0 animate-fade-in [--animation-delay:600ms]" ref={rowRef}>
          <Button
            ...
            className="w-[128px] md:w-[140px] h-[40px] md:h-[43px] bg-[#d5d7e3] hover:bg-[#e5e7f3] rounded-[18px] [font-family:'Gilroy-Medium-Medium',Helvetica] font-medium text-[#0d0d0d] text-sm md:text-base flex items-center justify-center gap-1.5"
            title={busyTitle}
            disabled={isBusy}
          >
            {accountId ? (
          ...
          {/* Dropdown (only when connected & toggled) */}
          {accountId && menuOpen && (
            <div
              role="menu"
