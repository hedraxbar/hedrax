...
export const NavigationSection: React.FC = () => {
  ...
  const hasAutoOpenedRef = React.useRef(false); // ensure deep link opens only once
  ...
  const resetPairingUI = React.useCallback(() => {
    try { wcClient?.clearDisplayUri?.(); } catch {}
  ...
  // SAFER deep-link opener:
  // - For auto-open (effect), navigate the SAME TAB to avoid popup blockers.
  // - For user-click (button), we still use window.open in a new tab.
  const openDeepLink = (uri: string | null, inNewTab = false) => {
    if (!uri) return;
    try {
      if (inNewTab) {
        // user gesture -> allowed to open a new tab
        window.open(uri, "_blank", "noopener,noreferrer");
      } else {
        // auto open -> same tab to avoid popup blocker about:blank
        window.location.href = uri; // extension usually intercepts; mobile opens app
      }
    } catch { /* noop */ }
  };

  // When displayUri appears → open pairing modal, build QR, auto-open deep link ONCE (same tab)
  React.useEffect(() => {
    let cancelled = false;
    async function handleUri() {
      ...
      // Auto-open deep link in SAME tab to avoid popup blocker about:blank
      if (!hasAutoOpenedRef.current) {
        hasAutoOpenedRef.current = true;
        // slight delay helps some browsers/extensions register listeners
        setTimeout(() => openDeepLink(displayUri, false), 50);
      }
    }
    handleUri();
  ...
  async function chooseWallet(
    ...
    try {
      ...
      // (Optional) If URI is already there very quickly, also navigate same tab
      if (displayUri && !hasAutoOpenedRef.current) {
        hasAutoOpenedRef.current = true;
        openDeepLink(displayUri, false);
      }
    } finally {
      setIsActing(false);
    }
              ...
              <div>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  <button
                    ...
                    onClick={() => openDeepLink(displayUri, true)} // user-click → new tab OK
                  >
